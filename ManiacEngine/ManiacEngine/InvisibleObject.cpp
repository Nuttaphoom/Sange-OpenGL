...
int InvisibleObject::Collide_W_Entity(Entity e) {
	...
	if (abs(e.GetPos().x - this->GetPos().x) > 140 || abs(e.GetPos().y - this->GetPos().y) > 140)
		return 0; 
	...
	float TOP_Y = (float)e.GetPos().y + e.collisionSize.y / 2 * -1; 

	float Middle_1_2_X = (float)e.GetPos().x - e.collisionSize.x / 2 ; 
 
	float Middle_1_Y = (float)e.GetPos().y + e.collisionSize.y * -1 / 4; 

 	float Middle_2_Y = (float)e.GetPos().y - e.collisionSize.y * -1 / 4; 

	float BOTTOM_Y = (float) e.GetPos().y - 1 - e.collisionSize.y  * -1 / 2 ;
...
int InvisibleObject::Collide_W_Object(DrawableObject* e) {
	int CollideDetection = 0; //Check where it collide with Entity (In Entity POV) 
						  // 1 FOR TOP, 2 FOR BOTTOM, 4 FOR LEFT, AND 8 FOR RIGHT 
	...
	float TOPY_Inv_Obj = (float)this->GetPos().y + this->GetSize().y  / 2;
	float BOTTOMY_Inv_Obj = (float)this->GetPos().y - this->GetSize().y / 2;

	float TOP_BOTTOM_X = (float)e->GetPos().x - e->GetSize().x / 4;
	float TOP_Y = (float)e->GetPos().y + e->GetSize().y / 2 ;

	float Middle_1_2_X = (float)e->GetPos().x - e->GetSize().x / 2;

	float Middle_1_Y = (float)e->GetPos().y + e->GetSize().y  / 4;

	float Middle_2_Y = (float)e->GetPos().y - e->GetSize().y  / 4;

	float BOTTOM_Y = (float)e->GetPos().y - 1 - e->GetSize().y  / 2;
